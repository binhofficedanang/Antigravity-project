...
class WebAutomation:
    ...
    def get_adapter(self, site_key):
        if not hasattr(self, "_adapters"):
            self._adapters = {}
        if site_key not in self._adapters:
            if site_key == "raovat.net":
                from adapters.site_adapters import RaoVatNetAdapter
                self._adapters[site_key] = RaoVatNetAdapter(self)
            elif site_key == "thuviennhadat.vn":
                from adapters.site_adapters import ThuvienNhaDatAdapter
                self._adapters[site_key] = ThuvienNhaDatAdapter(self)
        return self._adapters.get(site_key)
