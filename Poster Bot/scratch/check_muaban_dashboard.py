...
def main():
    print("Checking muaban.net dashboard...")
    ...
    try:
        ...
        print(f"Current Title: {bot.page.title()}")

        # Capture screenshot
        ss_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "muaban_dashboard.png")
        ...
        listings = bot.page.evaluate("""() => {
            const results = [];
            // Let's look for listing cards or items
            const cards = document.querySelectorAll('[class*="card"], [class*="item"], tr, li');
            cards.forEach(card => {
